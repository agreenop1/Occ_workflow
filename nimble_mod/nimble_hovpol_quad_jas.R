...
# MCMC settings
ni <- 100; nt <- 5 ; n.species="all"; daisy=T # can be all
...
win.data <-    list(y = y , 
                    ...
                    nclosure = nclosure, nsites = nsites, nspecies = ncol(y),
                    SHORT = visit$SHORT, 
                    LONG = visit$LONG,
                    nobs = nrow(occup),
                    COVS=cov.array,
...
mod. <- nimbleModel(code = modx, name = "modx",  constants = win.data,
                    inits = inits,calculate = F)

conmod <- configureMCMC(mod.,monitors=params,thin=nt)
...
out <- list(parameters=params,modState=stateList,iterations=ni,modData=win.data,inits=inits,
            chain=i,id=id,nimbleModel=modx,thin=nt,index=1)
