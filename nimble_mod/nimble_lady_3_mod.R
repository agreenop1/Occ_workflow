...
modcon <- configureMCMC(mod.)
modcon$addMonitors("phi")

# build mcmc - in this case all the default samplers are used
modmcmc <- buildMCMC(modcon)
...
# remove samples
cmcmc$run(niter=100,reset=F,resetMV = T)
samples <-rbind(samples,  as.matrix(cmcmc$mvSamples))
s<- read.csv('Occ_workflow_V2/nimble_mod/samples1.csv')[-1]
ggplot() + geom_line(aes(x=1:nrow(samples),y=logit(samples[,1])))
write.csv(samples,'Occ_workflow_V2/nimble_mod/samples1.csv')

# save the state of the model
stateList <- list(modelState = getModelState(cmod),
                  mcmcState = getMCMCstate(modcon, cmcmc))  

saveRDS(stateList, file = 'Occ_workflow_V2/nimble_mod/state.rds')

####################################################################
# create new model object
mod1 <- nimbleModel(code = modx, name = "mod1",  constants = win.data,
                   inits = inits)
modcon1 <- configureMCMC(mod1)
modmcmc1 <- buildMCMC(modcon1)

# compile nimble objects
cmod1 <- compileNimble(mod1)
cmcmc2 <- compileNimble(modmcmc1,project = mod1)

## load the saved "state" file
stateList <- readRDS(file = 'Occ_workflow_V2/nimble_mod/state.rds')

modelState <- stateList$modelState
mcmcState <- stateList$mcmcState

## restore the saved "state" into the new model and new MCMC
setModelState(cmod1, modelState)
setMCMCstate(modcon1, cmcmc2, mcmcState)

# remove samples
cmcmc2$run(niter = 5000,reset=F)
colnames(s) <- colnames(samples)
samples <- as.matrix(cmcmc2$mvSamples)[1:5000,]
s1<- rbind(s,samples[-1,])
ggplot() + geom_line(aes(x=1:14998,y=logit( s1[-1,1])))+xlim(8888,12888)
