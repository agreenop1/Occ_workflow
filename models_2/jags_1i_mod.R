...
covs <- c(base[[mod]]) 
inters <- list(c(1,4),c(2,3)) # can supply a vector of which variables have interactions based on position in above list
cat(names(covs),"\n")
######################################################################
...
if(!is.null(inters)){
  n.int <- length(inters)
  coms <- do.call(cbind,lapply(inters,function(x){ combn(x,2)})  )
