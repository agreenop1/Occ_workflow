...
out <- comb_daisy(parameters=c("alpha.phi","mu.alpha.phi","tau.alpha.phi","alpha.p",
                               ...
                               "init.occ","deviance"),
                  iter.index=1:20,chain.index=1:3,summary=T,file.path=file.path,by.it=1000,iterations=10000,verbose=T)
...
plot_effects("Temperature anomaly (deviation from mean of site)",species=species,cov=4)
...
rq <- plot_effects("Sum risk quotient",species=species,cov=4)
...
ggsave(paste0("spec_plot/mu_rq_",file,".png"),plot=par.p$mu.beta$mu.beta[[4]],
       width = wid,height = hei)
