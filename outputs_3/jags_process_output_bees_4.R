...
out <- comb_daisy(parameters=c("alpha.phi","mu.alpha.phi","tau.alpha.phi","alpha.p",
                               ...
                               "init.occ"),
                  iter.index=1:10,chain.index=1:3,summary=T,file.path=file.path,by.it=1000,iterations=2000,verbose=T)
...
covs <- c("Temperature_anom","Temp_mean","Semi.natural","RQsum")
...
if(length(dim(rhat))>1){
  ...
  for (b in 1:length(covs)){
    ...
    for (i in 1:nspecies){
      ...
      par  <- paste0(x,"[",b,",",i,"]")
      tr <- ggs_traceplot(samp.df[samp.df$Parameter==par,])+ggtitle(paste(species[i], "Rh",round(rhat[b,i],2)))
...
par.plots
  ...
  for (i in 1:par.l){
    ...
    tr <- ggs_traceplot(samp.df[samp.df$Parameter==par,])+ggtitle(paste("Rh =",r))
    ds <- ggs_density(samp.df[samp.df$Parameter==par,])+ 
      geom_vline(xintercept = 0, linetype="dashed", 
                 color = "black", size=0.5)
  ...
  for (i in 1:nspecies){
    ...
    par  <- paste0(x,"[",i,"]")
    tr <- ggs_traceplot(samp.df[samp.df$Parameter==par,])+ggtitle(paste(species[i], "Rh",round(rhat[i],2)))
    ds <- ggs_density(samp.df[samp.df$Parameter==par,])+ 
      geom_vline(xintercept = 0, linetype="dashed", 
                 color = "black", size=0.5)
...
plot_effects("RQ",species=species)
...
for (i in 1:length(species)){print(par.plots[[i]]$RQsum)}
plot(out,"alpha.phi")
