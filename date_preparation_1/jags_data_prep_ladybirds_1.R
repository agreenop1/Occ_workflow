...
# remove h. axyridis as it is an invasive species
unique(occ.dat$CONCEPT)

occ.dat <- occ.dat[occ.dat$CONCEPT!="Harmonia axyridis",]
...
id_plots <- lapply(covars_id,function(x1){
  
  x1 <- left_join(x1,coord)
  
  
  ep <- lapply(cov_year,function(x,x1){
    ...
    ggplot() +
      ...
      scale_fill_continuous(type = "viridis", name = x)
    
  },x1=x1)
  
  ggpubr::ggarrange(plotlist =  ep,ncol=3,nrow=1)
})
...
lapply(cors_mat,function(x){
  ...
  ggsave(paste0("spec_plot/",cmx[1,3],"_",cmx[1,4],".png")
...
ggsave("spec_plot/RQts.png",plot=time_trends$RQsum,width = 8,height = 2)
ggsave("spec_plot/AGts.png",plot=time_trends$agri,width = 8,height = 2)
ggsave("spec_plot/SEts.png",plot=time_trends$semi,width = 8,height = 2)
...
covars=readRDS("Model_data/data_bees_all.499_1994.2010.rds")
covars=covars[[4]]
covars$RQsum_A
covars$RQsum_M