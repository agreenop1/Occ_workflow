...
# urban
urban <- var_subset(env,"Urban.areas",smry=summary.env)
colnames(urban)[4] <- "year" 
...
# all semi
grass <- var_subset(env,"Grassland",smry=summary.env)
colnames(grass)[4] <- "year"

# temp anom
tree <- var_subset(env,"Tree_shrub",smry=summary.env)
colnames(tree)[4] <- "year"

a=distinct(m.temp[c(2,3)])
b=distinct(env[c(1,4)])
all(a[order(a$gr),]==b[order(b$gr),])

env_covar <- list(temp_anom=temp.a,mean_temp=m.temp,agri=agri,semi=semi,tree=tree,grass=grass,urban=urban)
