...
var.x <- c("mean.x","mean.x","mean.x","mean.x","mean.x","mean.x","predators_pollinators_RQ") # variable of interest CHECK!
...
zobs.x <- table(o$CONCEPT, o$site_5km, o$closure_per)
zobs.x[zobs.x>1] <- 1
zobs.x <- ifelse(zobs.x>0,1,NA)

# zobs
obs <- cbind(occup,visit)
 
obs.l <- pivot_longer(obs,cols =colnames(occup[-1]),names_to = "CONCEPT" )
obs.l$value[obs.l$value==TRUE] <- 1
...
saveRDS(list(occup,visit,zobs,covars,closure.period,zobs.x),file.name)



ggpubr::ggarrange(env_plots$RQsum,env_plots$tree,env_plots$grass
                  , nrow=3)
env_plots
...
cov.l <-list()
for (i in 1:length(covars_id)){
colnames(covars_id[[i]])[2:10] <- 2:10
cov.l[[i]] <- pivot_longer(covars_id[[i]],cols =2:10,names_to = "TP")
cov.l[[i]]$TP  <- as.numeric(cov.l[[i]]$TP)
colnames(cov.l[[i]])[c(1,3)] <- c("site_5km", names(covars_id)[i] ) 
obs.l <- left_join(obs.l,cov.l[[i]])
obs.l$value[obs.l$value==T] <- FALSE 
}

m1<- lme4::glmer(value~RQsum + agri+grass+tree+(1|TP)+(1|site_5km)+(1|CONCEPT),family = binomial, data=obs.l)
