...
var.x <- c("mean.x","mean.x","mean.x","mean.x","predators_RQ") # variable of interest CHECK!
...
#zobs <- array (dim=c(ncol(occup[-1]),nrow(site_id),length(unique(visit$TP))))
#for (i in 1:length(site_id$site_name)){
#  for (t in 1:length(unique(visit$TP))){
#    for (s in 1:length(colnames(occup[-1]))){
#      
#      tp <- t
#      si <- site_id$site_name[i]
#      sp <- colnames(occup[-1])[s]
#      
#      s.set <- visit[visit$TP%in%tp , ]
#      s.set <- s.set[s.set$site_5km%in%si,]
#     
#       if(nrow(s.set)==0){
#    
#      zobs[s,i,t] <-   NA 
...
#     }else{
#      
#      o.set <- occup[occup$visit%in%s.set$visit,]
#      o.set[o.set==TRUE] <- 1
#      zobs[s,i,t] <- ifelse (sum(o.set[,sp])>0,1,0)
#       
#     }
#    
#    }
...
saveRDS(list(occup,visit,zobs,covars,closure.period,site_id),file.name)
